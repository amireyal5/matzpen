...
export default function LandingScreen({ onComplete, onGoToAuth, initialName = "", initialGender = "m" }: LandingScreenProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-[linear-gradient(135deg,#0F172A_0%,#1E293B_50%,#0F172A_100%)] overflow-y-auto">
      <div className="fixed top-[10%] right-[10%] w-48 h-48 rounded-full border border-indigo-500/10 pointer-events-none" aria-hidden="true" />
      <div className="fixed bottom-[15%] left-[5%] w-72 h-72 rounded-full border border-indigo-500/5 pointer-events-none" aria-hidden="true" />
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-[2.5rem] p-8 border border-indigo-500/20 shadow-2xl space-y-6">
          <div className="space-y-4">
             <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label htmlFor="landing-name" className="text-[10px] font-bold text-slate-500 tracking-[0.2em] uppercase block pr-1">איך נקרא לך?</label>
                <input
                  id="landing-name"
                  value={name}
