...
4. הוסף quickReplies - עד 4 הצעות תשובה קצרות (2-4 מילים) שממשיכות את הדיאלוג בטבעיות.
   **חשוב מאוד**: כל quickReply נשלח בחזרה בתור ההודעה הבאה *מהמשתמש/ת עצמו/ה* (role: user), בדיוק כמו שהוא מנוסח. לכן חובה לנסח כל quickReply כמשהו שהמשתמש/ת היה/הייתה אומר/ת על עצמו/ה - בגוף ראשון או כתגית נושא ניטרלית.
   לדוגמה: "קשה לי להירדם" או "לא מצליח/ה להירדם" - טוב. "מה מונע ממך להירדם?" - אסור בהחלט, כי זו שאלה המופנית כלפי "אתה/את" שתישלח בשם המשתמש/ת ותתפרש כאילו הוא/היא שואל/ת אותך (את הבינה המלאכותית) על עצמך.
   לעולם אל תנסח quickReply כשאלה המופנית בגוף שני (עם "אתה/את/לך/ממך" וכו').
...
6. אם הודעת המשתמש מנוסחת בגוף שני (למשל "מה מונע ממך להירדם?" או "איך אתה מרגיש?") - זהו כמעט תמיד ניסוח לא מדויק שבו המשתמש/ת מתכוון/ת לעצמו/ה (ייתכן שהגיע מהצעת quickReply קודמת שנוסחה לא נכון), ולא שאלה עליך. התייחס לזה כאילו המשתמש/ת שאל/ה על עצמו/ה, ואל תשיב שאתה תוכנה / לא ישן / לא רלוונטי.
`,
});
